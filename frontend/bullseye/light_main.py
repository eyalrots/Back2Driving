...
# Assumes structure: Project/Back2Driving/frontend/bullseye/light_main.py
# access_shared.py is in frontend/ (one level up)
...
class MultiTarget(QGraphicsItemGroup):
    def __init__(self, y_pos, x_offset=0):
        ...
        self.addToGroup(self.outer); self.addToGroup(self.middle); self.addToGroup(self.inner)
...
class PlayerCar(QGraphicsItemGroup):
    def __init__(self):
        ...
        grad.setColorAt(0, QColor(0, 100, 255)); grad.setColorAt(1, QColor(0, 200, 255))
        ...
        self.addToGroup(body); self.addToGroup(self.left_light); self.addToGroup(self.right_light)
        self.setZValue(100)

    def set_braking(self, is_braking):
        color = QColor(255, 0, 0) if is_braking else QColor(80, 0, 0)
        self.left_light.setBrush(QBrush(color)); self.right_light.setBrush(QBrush(color))
...
class MainMenu(QWidget):
    ...
    def __init__(self):
        ...
        self.name_input.setFixedWidth(300); self.name_input.setFocusPolicy(Qt.FocusPolicy.StrongFocus)
        layout.addWidget(self.name_input)
        layout.addWidget(QLabel("Select Difficulty:"))
        self.diff_box = QComboBox(); self.diff_box.addItems(["Easy", "Medium", "Hard"])
        self.diff_box.setFixedWidth(300); self.diff_box.setCurrentText("Medium")
        ...
        self.start_btn.setFixedWidth(300); self.start_btn.setFixedHeight(50)
        self.start_btn.setAutoDefault(True); self.start_btn.setFocusPolicy(Qt.FocusPolicy.StrongFocus)
        self.start_btn.clicked.connect(self.on_start)
        layout.addWidget(self.start_btn)

    def on_start(self):
        self.start_signal.emit(self.name_input.text().strip() or "Anonymous", self.diff_box.currentText())
...
class GameScreen(QWidget):
    ...
    def __init__(self):
        ...
        self.timer = QTimer(); self.timer.timeout.connect(self.update_game)
        self.countdown_timer = QTimer(); self.countdown_timer.timeout.connect(self.tick_time)

    def start_session(self, name, difficulty):
        self.patient_name, self.difficulty, self.config = name, difficulty, DIFFICULTY_SETTINGS[difficulty]
        self.scene.clear()
        self.score = 0; self.combo_count = 0; self.time_left = GAME_DURATION; self.current_speed = 0.0
        self.is_active = True
        self.reaction_times = []; self.ptt_times = []
        self.gas_release_time = None; self.active_target_spawn_time = None
        self.has_braked_for_current_target = False
        self.last_input_state = {"gas": False, "brake": False}
        self.gas_value = 0.0; self.brake_value = 0.0
        self.setup_environment()
        self.timer.start(16); self.countdown_timer.start(1000)

    def setup_environment(self):
        self.grass_patches = []
        for i in range(-1, 9):
            for x in [0, 550]:
                p = QGraphicsRectItem(0, 0, 250, 100); p.setPos(x, i * 100)
                p.setBrush(QBrush(QColor(30, 80, 30) if i % 2 == 0 else QColor(35, 90, 35)))
                p.setPen(QPen(Qt.PenStyle.NoPen)); self.scene.addItem(p); self.grass_patches.append(p)
        self.road_lines = []
        for i in range(-1, 11):
            line = QGraphicsRectItem(0, 0, 8, 35); line.setPos(WIDTH // 2 - 4, i * 70)
        ...
        self.player = PlayerCar(); self.player.setPos(WIDTH // 2 - 25, HEIGHT - 150)
        ...
        self.msg_bg.setBrush(QBrush(QColor(0, 0, 0, 180))); self.msg_bg.setPen(QPen(Qt.GlobalColor.white, 1))
        self.msg_bg.hide(); self.scene.addItem(self.msg_bg)
        self.msg_text = self.create_hud("", WIDTH // 2, 230, "white", 20); self.msg_text.setZValue(200)
        self.targets = []
    ...
    def handle_keyboard_input(self, key, is_pressed):
        if is_pressed: self.keys_pressed.add(key)
    ...
    def update_game(self):
        ...
        GAS_MIN = 2122; GAS_MAX = 3200; BRAKE_MIN = 0; BRAKE_MAX = 8388608
...
class ResultsScreen(QWidget):
    ...
    def __init__(self):
        ...
        hint.setFont(QFont("Segoe UI", 12, QFont.Weight.Bold)); hint.setStyleSheet("color: #AAAAAA; margin-bottom: 10px;")
        hint.setAlignment(Qt.AlignmentFlag.AlignCenter); layout.addWidget(hint)
        title = QLabel("CLINICAL SESSION HISTORY")
        title.setFont(QFont("Segoe UI", 24, QFont.Weight.Bold)); title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(title)
        self.table = QTableWidget(0, 7)
        self.table.setHorizontalHeaderLabels(["Attempt", "Date/Time", "Name", "Difficulty", "Score", "Avg Reaction", "Avg PTT"])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.table.setFocusPolicy(Qt.FocusPolicy.NoFocus); layout.addWidget(self.table)
        btn_layout = QHBoxLayout()
        self.play_again_btn = QPushButton("PLAY AGAIN")
        self.play_again_btn.setFixedHeight(50); self.play_again_btn.setAutoDefault(True)
        ...
        self.home_btn.setFixedHeight(50); self.home_btn.setAutoDefault(True)
        self.home_btn.setFocusPolicy(Qt.FocusPolicy.StrongFocus)
        self.home_btn.clicked.connect(lambda: self.home_signal.emit())
        btn_layout.addWidget(self.play_again_btn); btn_layout.addWidget(self.home_btn)
...
class BullseyeApp(QMainWindow):
    def __init__(self):
        ...
        # --- NEW: ROOT LEVEL SUBPROCESS LAUNCH ---
        global sensor_process
        if HAS_SENSOR and os.path.exists(SENSOR_EXE_PATH):
            print(f"Launching C Sensor executable with sudo at: {SENSOR_EXE_PATH}")
            # The "sudo" command is added to the subprocess list
            sensor_process = subprocess.Popen(["sudo", SENSOR_EXE_PATH], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            time.sleep(0.5) 
        elif HAS_SENSOR:
            print(f"Error: Could not find C executable at {SENSOR_EXE_PATH}")
    ...
    # --- NEW: CLEANUP ON NORMAL APP CLOSE ---
